...
import { positionStats, seasonStats } from "@/lib/pinball/LeaderboardStats";
...
export default function PlayerSummaryData(data, username) {
  ...
  if (user) {
    ...
    userPositionData = positionWeeksData.map((week) => ({
      ...week,
    ...
    const sortedScoresDescending = [...positionWeeksData[0].scores].sort(
      (a, b) => b.winPercentage - a.winPercentage
    );
    const sortedPlayerIndex = sortedScoresDescending.findIndex(
      (s) => s.username === username
    );
    const higherRankScore =
      sortedPlayerIndex >= 1
        ? sortedScoresDescending[sortedPlayerIndex - 1]
        : undefined;
    const lowerRankScore =
      sortedPlayerIndex >= 0 &&
      sortedPlayerIndex < sortedScoresDescending.length
        ? sortedScoresDescending[sortedPlayerIndex + 1]
        : undefined;
    playerRivals = [{ ...higherRankScore }, { ...lowerRankScore }].filter(
