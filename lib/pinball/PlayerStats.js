...
export function userWinPercentage(data, username) {
  const winPercentage = Number(
    (
      (playerWins(data, username) /
        numberOfPlayersWithScores(data, username)) *
      100
    ).toFixed(2)
...
function userSummaryData(positionWeeksData, seasonWeeksData, username) {
  ...
  user.annualWinPercentage = userWinPercentage(positionWeeksData, username);

  user.seasonWinPercentage = userWinPercentage(seasonWeeksData, username);
  return user;
}
...
export default function PlayerSummaryData(data, username) {
  ...
  let userSeasonDetails = seasonWeeksData[0].scores.find(
