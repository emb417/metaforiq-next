...
import { GiPayMoney, GiSmallFire, GiBlackball, GiBowlingPin } from "react-icons/gi";
...
<h1 className="flex text-2xl text-white m-4 min-w-[max-content]"><GiBlackball className='text-3xl mr-2' />Libowski<GiBowlingPin className='text-3xl mx-1' />The Library Dude</h1>
      <div className="flex items-center text-2xl text-white m-4">
        <ul className="flex flex-wrap w-full gap-4">
          {navItems.map(item => (
            <Link key={item.id} href={item.href} className='w-full md:w-[max-content] hover:text-teal-300 duration-100'>
