...
export default function WishListAvailability( { availability } ) {
    ...
    return (
        ...
        {Object.keys(availability || {}).map((availId) => {
          ...
          const notifyDate = avail.notifyDate ? new Date(avail.notifyDate * 1000) : "";
          const timeSinceNotifyDate = notifyDate ? Math.floor((Date.now() - notifyDate) / (1000 * 60)) : "";
          return (
            <div
            ...
            >
              ...
              >
                {avail.location}<br/>{notifyDate.toLocaleString()}
              </div>
            </div>
          );
        })}
